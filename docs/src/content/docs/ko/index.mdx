...
- **실시간 알림**
  - WebSocket API + ElastiCache Valkey로 실시간 상태 업데이트
...
├── StorageStack          - S3 버킷, DynamoDB 테이블, ElastiCache Valkey
...
워크플로우 진행 상태가 DynamoDB에 기록되면, DynamoDB Streams가 변경을 감지합니다. VPC 내부의 WorkflowStream Lambda가 Valkey에서 활성 연결을 조회한 후 WebSocket API로 이벤트를 푸시하여, 프론트엔드에서 실시간으로 상태를 반영합니다.
...
| ElastiCache Valkey | WebSocket 연결 상태 관리 (DynamoDB TTL 대비 고속) |
