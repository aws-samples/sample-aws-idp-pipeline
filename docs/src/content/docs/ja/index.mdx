...
- **[ハイブリッド検索](./vectordb.md)**
  ...
  - Lindera / ICU4Xトークナイザーによるキーワード抽出
...
| PaddleOCRデュアルバックエンド（Lambda + SageMaker） | CPUモデル（PP-OCRv5）はRust Lambda、GPUモデル（VL）はSageMaker |
...
| モデル | 用途 | 説明 |
|--------|------|------|
| PP-OCRv5 | OCR（CPU） | Rust Lambda（MNNベース）、汎用テキスト抽出 |
...
### Lambdaサービス（Rust）
- Lindera / ICU4X（多言語トークナイザー）
- LanceDB Service（ベクトル検索 + FTS）
- PaddleOCR（MNNベースCPU推論）
