...
- **リアルタイム通知**
  - WebSocket API + ElastiCache Valkeyによるリアルタイムステータス更新
...
+-- StorageStack          - S3バケット、DynamoDBテーブル、ElastiCache Valkey
...
ワークフローの進行状態がDynamoDBに記録されると、DynamoDB Streamsが変更を検出します。VPC内部のWorkflowStream LambdaがValkeyでアクティブ接続を照会し、WebSocket APIを通じてイベントをプッシュすることで、フロントエンドでリアルタイムにステータスを反映します。
...
| ElastiCache Valkey | WebSocket接続状態管理（DynamoDB TTLより高速） |
