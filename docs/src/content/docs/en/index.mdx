...
- **Real-time Notifications**
  - Real-time status updates via WebSocket API + ElastiCache Valkey
...
+-- StorageStack          - S3 buckets, DynamoDB tables, ElastiCache Valkey
...
When workflow progress is recorded in DynamoDB, DynamoDB Streams detects the changes. The WorkflowStream Lambda inside the VPC looks up active connections in Valkey, then pushes events through the WebSocket API so the frontend reflects status in real time.

```
DynamoDB Streams (state change detection)
  -> WorkflowStream Lambda (VPC)
    -> Valkey (connection lookup)
...
| ElastiCache Valkey | WebSocket connection state management (faster than DynamoDB TTL) |
