...
#[tokio::test]
#[ignore]
async fn test_action_search_keywords() {
    init_tracing();
    dotenvy::dotenv().ok();
    let conn = db::connect().await.unwrap();
    let aws_config = aws_config::load_defaults(aws_config::BehaviorVersion::latest()).await;
    let bedrock_client = aws_sdk_bedrockruntime::Client::new(&aws_config);
    let output = search_keywords::execute(
        &conn,
        &bedrock_client,
        search_keywords::SearchKeywordsParams {
            project_id: "proj_HLEpYD_QD5iT6VwptGxYJ".to_string(),
            query: "떡볶이".to_string(),
            limit: None,
        },
...
async fn test_action_add_graph_keywords() {
    ...
    let output = add_graph_keywords::execute(
        ...
        add_graph_keywords::AddGraphKeywordsParams {
            ...
            keywords: vec![
                "떡볶이", "김치찌개", "불고기", "비빔밥", "잡채",
                "갈비탕", "삼겹살", "된장찌개", "순두부찌개", "냉면",
                "칼국수", "만두", "김밥", "떡국", "제육볶음",
                "닭갈비", "감자탕", "부대찌개", "해물파전", "잔치국수",
                "콩나물국", "미역국", "설렁탕", "육개장", "갈비찜",
                "족발", "보쌈", "치킨", "짜장면", "짬뽕",
                "탕수육", "볶음밥", "라면", "우동", "소바",
                "초밥", "회덮밥", "돈까스", "카레", "오므라이스",
                "파스타", "피자", "햄버거", "샌드위치", "스테이크",
                "샐러드", "수프", "리조또", "그라탕", "크로켓",
                "타코", "부리또", "나초", "퀘사디아", "엔칠라다",
                "팟타이", "쌀국수", "똠양꿍", "카오팟", "솜탐",
                "커리", "난", "탄두리치킨", "비리야니", "사모사",
                "딤섬", "마파두부", "깐풍기", "양장피", "유린기",
                "라멘", "규동", "오코노미야키", "타코야키", "야키소바",
                "훠궈", "마라탕", "딴딴면", "충칭소면", "꿔바로우",
                "케밥", "후무스", "팔라펠", "샤와르마", "바클라바",
                "크루아상", "바게트", "마카롱", "에클레어", "타르트",
                "티라미수", "판나코타", "젤라토", "카놀리", "브루스케타",
                "프레첼", "슈니첼", "브라트부르스트", "굴라쉬", "슈트루델",
