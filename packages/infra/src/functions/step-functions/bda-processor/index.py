...
def handler(event, _context):
    ...
    if not all([document_id, file_uri, file_type]):
        return {
            'statusCode': 400,
            'body': 'Missing required fields: document_id, file_uri, file_type'
        }
    ...
    try:
        ...
        output_prefix = f'bda-output/{document_id}/{datetime.now(timezone.utc).strftime("%Y%m%d-%H%M%S")}'
